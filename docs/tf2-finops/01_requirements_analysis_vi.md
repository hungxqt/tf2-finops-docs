...
Đội ngũ CDO chịu trách nhiệm về FinOps control plane, xây dựng kiến trúc lakehouse-centric để ingest và xử lý dữ liệu chi phí (S3 CUR partition pulls và Cost Explorer API calls), workflow điều phối (orchestration), quản lý trạng thái vận hành, hiển thị dashboard, định tuyến cảnh báo (alert routing), thiết lập các containment guardrails, và ghi nhật ký kiểm toán (audit logs). Đội ngũ CDO cũng triển khai và vận hành hạ tầng host Lambda container (VPC, subnets, Private REST API Gateway với IAM SigV4, các hàm Lambda, execution roles, reserved concurrency, các hàng đợi bất đồng bộ SQS/DLQ, và DynamoDB state stores) cho một endpoint AI Engine của Task Force dùng chung, được truy cập an toàn qua endpoint Private API Gateway với xác thực IAM SigV4. Các workload được chạy dưới dạng thực thi container image serverless trên AWS Lambda, được tối ưu hóa với SQS cho xử lý bất đồng bộ. AIOps sở hữu container image AI Engine tương thích với Lambda, mã nguồn mô hình, logic phát hiện, văn bản giải thích và các chỉ số đánh giá backtest.
...
| AI Engine hosting uptime | ≥99.5% availability cho hosted model API | API AI Engine do CDO host qua Private API Gateway phải đáng tin cậy cho các tác vụ gọi thực thi. |
...
| Nền tảng Private API Gateway & Lambda Container (Cấu hình gateway, tích hợp VPC, Execution Roles) | Owns | |
...
*Ghi chú: Đội ngũ CDO tiêu thụ (consume) AI Engine thông qua một endpoint Private API Gateway dùng chung, được truy cập an toàn sử dụng xác thực IAM SigV4 và tích hợp Lambda proxy/container. Ranh giới trách nhiệm được xác định cụ thể: CDO sở hữu và triển khai hạ tầng host (VPC, subnets, Private REST API Gateway, các hàm Lambda, execution roles, reserved concurrency, các hàng đợi bất đồng bộ SQS/DLQ, và DynamoDB/S3 stores), trong khi AIOps sở hữu container image AI Engine tương thích với Lambda, mã nguồn mô hình, logic phát hiện, văn bản giải thích và các chỉ số đánh giá backtest. Telemetry gửi tới AI Engine chỉ gồm dữ liệu chi phí CUR và Cost Explorer API, loại bỏ hoàn toàn các chỉ số hiệu năng CloudWatch (CPU, memory, database connections) vốn chỉ dành riêng cho việc giám sát vận hành của CDO.*
...
| **Tính khả dụng hệ thống** | >=99.5% | Tổng thời gian hoạt động của Private API Gateway dành cho CDO. |
...
- [ ] **Budget ceiling**: Hạn mức ngân sách tối đa dành cho CDO Lambda container hosting platform (Private API Gateway, thời gian chạy của Lambda, hàng đợi SQS/DLQ, và các tùy chọn Provisioned Concurrency) trong thời gian chạy capstone là bao nhiêu?

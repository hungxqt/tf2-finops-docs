...
| `POST /v1/audit/{audit_id}/rollback` | `ai-api-contract.md` | Giao diện thông báo kết quả rollback. Ghi lại kết quả thực hiện rollback sau khi CDO đã tự thực hiện rollback từ payload Boto3 được cache, cập nhật các metric error budget của tenant. |
...
- **AIOps API contract freeze**: Hợp đồng đã được ký và đóng băng vào ngày 2026-06-25; mọi thay đổi tiếp theo cần một Yêu cầu Thay đổi Chính thức (Formal Change Request).

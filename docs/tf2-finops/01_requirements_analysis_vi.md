...
Đội ngũ CDO chịu trách nhiệm về FinOps control plane, xây dựng kiến trúc lakehouse-centric để ingest và xử lý dữ liệu chi phí (S3 CUR partition pulls và Cost Explorer API calls), workflow điều phối (orchestration), quản lý trạng thái vận hành, hiển thị dashboard, định tuyến cảnh báo (alert routing), thiết lập các containment guardrails, và ghi nhật ký kiểm toán (audit logs). Đội ngũ CDO cũng triển khai và vận hành hạ tầng host Lambda container (VPC, subnets, các hàm Lambda, execution roles, reserved concurrency, các hàng đợi bất đồng bộ SQS/DLQ, và DynamoDB/S3 state stores) để chạy AI Engine do AIOps cung cấp. Trong workflow chạy batch theo lịch trình mặc định, bộ điều phối trung tâm Step Functions gọi trực tiếp hàm AI Engine Request Lambda, hàm này sẽ đẩy yêu cầu vào hàng đợi SQS. Hàm AI Engine Worker Lambda xử lý bất đồng bộ các tác vụ từ hàng đợi SQS và ghi kết quả trực tiếp vào DynamoDB và S3. Lưu ý rằng `/v1/detect` và `/v1/detect/result/{audit_id}` đại diện cho ngữ nghĩa hợp đồng logic (logical contract semantics) cho việc tích hợp mô hình, không phải các đường dẫn REST/HTTP được triển khai thực tế trong workflow batch cơ sở này, do không triển khai Private API Gateway. Các workload được chạy dưới dạng thực thi container image serverless trên AWS Lambda, được tối ưu hóa với SQS cho xử lý bất đồng bộ. AIOps sở hữu container image AI Engine tương thích với Lambda, mã nguồn mô hình, logic phát hiện, văn bản giải thích và các chỉ số đánh giá backtest.
...
*Ghi chú: Đội ngũ CDO tiêu thụ (consume) AI Engine thông qua một hợp đồng lập trình có phiên bản qua việc gọi trực tiếp hàm Lambda (đại diện cho các ngữ nghĩa logic `/v1/detect` và `/v1/detect/result/{audit_id}`). Ranh giới trách nhiệm được xác định cụ thể: CDO sở hữu và triển khai hạ tầng host (VPC, subnets, các hàm Lambda, execution roles, reserved concurrency, các hàng đợi bất đồng bộ SQS/DLQ, và DynamoDB/S3 stores), trong khi AIOps sở hữu container image AI Engine tương thích với Lambda, mã nguồn mô hình, logic phát hiện, văn bản giải thích và các chỉ số đánh giá backtest. Telemetry gửi tới AI Engine chỉ gồm dữ liệu chi phí CUR và Cost Explorer API, loại bỏ hoàn toàn các chỉ số hiệu năng CloudWatch (CPU, memory, database connections) vốn chỉ dành riêng cho việc giám sát vận hành của CDO.*

*Ghi chú về Ngôn ngữ Hợp đồng (Contract Wording)*: Hợp đồng API đã ký kết (`ai-api-contract.md`) được thiết lập trên giả định ban đầu về việc lưu trữ trên ECS/ALB. Vì kiến trúc nền tảng đã chuyển sang Lambda container hosting, mọi ngôn ngữ cũ về ECS hoặc ALB trong bản hợp đồng đã ký đó sẽ yêu cầu một yêu cầu thay đổi hợp đồng chính thức (change request) khi hợp đồng được cập nhật sau này. Hiện tại, tài liệu hợp đồng vẫn được giữ nguyên không đổi.
...
|---|---|---|
| **Độ trễ Ingestion (P99)** | < 50 ms | Thời gian xử lý hai chiều của yêu cầu gọi hàm trực tiếp. |
...
| **Tỷ lệ lỗi** | < 0.5% | Tỷ lệ phản hồi lỗi thực thi trên tổng số yêu cầu gọi hàm. |

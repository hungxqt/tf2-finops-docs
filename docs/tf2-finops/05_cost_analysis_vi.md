...
Một "tenant" trong ngữ cảnh TF2 FinOps Watch là một tài khoản thành viên AWS được giám sát chi phí. Mô hình chi phí phân tách chi phí biến đổi trên mỗi tenant khỏi chi phí nền tảng dùng chung. Sự phân biệt này rất quan trọng vì lakehouse và Lambda workflow chủ yếu mở rộng theo số lượng tài khoản/dữ liệu, trong khi Lambda container và giám sát cơ sở tạo ra chi phí cố định dùng chung cần được phân bổ khấu hao trên các tenant.

CDO sở hữu chi phí lưu trữ vận hành của AI Engine do AIOps cung cấp trên các hàm Lambda container: các hàm Lambda thực thi container, ECR, các Lambda execution roles, cấu hình secrets, reserved concurrency, các hàng đợi SQS/DLQ và giám sát runtime. AIOps sở hữu việc phát triển mô hình, thiết kế huấn luyện mô hình, chất lượng mô hình và bất kỳ bộ dữ liệu lịch sử tổng hợp nào được sử dụng để huấn luyện, cải tiến hoặc backtest mô hình. Nếu các tác vụ huấn luyện hoặc huấn luyện lại của AIOps chạy trên các hàm Lambda do CDO host, chi phí tính toán phải được gắn tag và báo cáo riêng là "chi phí workload AI Engine do CDO host".
...
| **Storage - S3 raw/curated** | $0.023/GB-tháng Standard, thấp hơn sau lifecycle | Kéo CUR/Cost Explorer, parquet chuẩn hóa, và trích xuất dashboard | Biến đổi theo dung lượng dữ liệu hóa đơn. |
| **Storage - S3 authoritative audit** | $0.0125/GB-tháng ước tính IA | Bằng chứng quyết định lưu trữ S3/Object Lock (lưu giữ ít nhất 90 ngày), sao lưu telemetry/lịch sử, và bằng chứng rollback | Biến đổi theo dung lượng cảnh báo/containment; việc lưu trữ là bắt buộc. |
| **Database - DynamoDB on-demand** | $1.25/triệu write + $0.25/triệu read | `finops-idempotency-{env}` (24h TTL), `finops-rollback-cache` (90-day TTL), và Dashboard Cache | Biến đổi theo các lượt chạy và lượt đọc dashboard. |
...
| **Data Catalog - Glue** | Lưu trữ catalog/yêu cầu metadata | Bảng chi phí, phân vùng, Partition Projection (ADR-014) | Biến đổi nhưng không đáng kể ở quy mô capstone (gói miễn phí; ADR-014). |
| **Compute - AI Engine Lambda** | $0.20/1M requests + $0.0000166667/GB-second | Hàm Lambda container AI Engine thực thi đồng bộ; chu kỳ 24h | Chi phí host workload AI biến đổi; gắn tag riêng biệt khỏi CDO adapter. |
| **Hàng đợi SQS & DLQ** | $0.40/triệu requests | Vùng đệm retry cho các request của Lambda alert routing | Chi phí vận hành hàng đợi biến đổi. |
...
| **Compute - Private ALB / HTTPS Adapter** | $0.0225/giờ + $0.008/LCU-giờ | Endpoint ALB nội bộ SigV4 để định tuyến tới Lambda (~$16.20/tháng cố định) | Chi phí định tuyến cố định dùng chung cho các endpoint /v1/* bảo mật. |
| **VPC endpoints** | Phí endpoint hàng giờ + xử lý dữ liệu nếu áp dụng | Các kết nối riêng tư cho ECR, S3, DynamoDB, Secrets Manager, Logs, KMS, và STS | Chi phí bảo mật cố định dùng chung. |
| **Secrets Manager** | $0.40/bí mật/tháng + phí request | Thông tin đăng nhập cơ sở dữ liệu dashboard, webhook, ID bên ngoài | Dùng chung cố định cộng với lượng request. |
...
| **Observability - CloudWatch, Prometheus, OTel & X-Ray** | Phí Logs, metrics, trace analyzer, bộ thu thập ADOT/OTel, và dashboard | Logs của Lambda, traces của Step Functions, metrics hàng đợi, các metric hiệu năng (CPU, Memory, database utilization) và dashboard giám sát nền tảng | Dùng chung và biến đổi; việc thu thập ADOT và telemetry có thể trở thành nhân tố dẫn đầu về chi phí. |
| **Query - Cost Explorer API** | $0.01 cho mỗi request | Chỉ thực hiện truy vấn fallback khi tệp CUR S3 bị trễ (`telemetry_delay_event = true`) | Chi phí truy vấn fallback có điều kiện; bình thường là $0 dưới hoạt động CUR tiêu chuẩn. |
| **Provisioned Concurrency (Tùy chọn)** | $0.015/GB-second + $0.15/1M requests concurrency charges | Các môi trường thực thi được làm ấm sẵn cho hàm Lambda container AI Engine | Tối ưu hóa sản xuất tùy chọn; `Cần bằng chứng: lượng concurrency yêu cầu và số giờ làm ấm`. |
...
- VPC endpoint, KMS và khả năng quan sát là những chi phí cố định lớn nhất.
...
- Việc bật tham số `callback_url` sẽ kích hoạt thêm chi phí truyền dữ liệu ra ngoài (egress), ghi nhật ký và chi phí thử lại (retry) khi các thông báo bất đồng bộ được bật.

---

## 2. Chi phí theo Quy mô (Cost at Scale)

Khi số lượng tenant tăng, một số chi phí cố định (VPC Endpoints, KMS CMKs, và dashboard S3 + CloudFront) được phân bổ giữa nhiều tenant, giảm chi phí trung bình mỗi tenant. Phần này sử dụng cấu trúc dự báo thay vì tuyên bố kết quả đo lường thực tế.
...
|---|---|---|---|---|---|
| **1** | `Cần bằng chứng: VPC endpoints` | `Cần bằng chứng: chi phí chạy 1 tài khoản` | `Cần bằng chứng: sử dụng AI Lambda` | `Cần bằng chứng` | `Cần bằng chứng` |
...
- 6× VPC Interface Endpoints (ECR, Logs, KMS, Secrets Manager, STS): $43.20 (với S3 và DynamoDB được cấu hình dưới dạng Gateway Endpoints miễn phí)
...
**Phân tích**:
- Các VPC endpoint cấu thành baseline nền tảng hỗ trợ việc host Lambda container ổn định, vùng đệm hàng đợi, khả năng quan sát và mạng riêng tư.
- Ở số lượng tenant lớn hơn, chi phí trung bình sẽ giảm xuống do các endpoint và chi phí dashboard được dùng chung.
...
| **Lambda reserved concurrency** |  Implemented | N/A | Cấu hình Reserved Concurrency cơ bản (5-10 thực thi đồng thời) đóng vai trò như chốt chặn chi phí/vùng ảnh hưởng lỗi (blast guardrail), trong khi Provisioned Concurrency là tùy chọn. |
...
| **Gộp lô tin nhắn SQS cho alert** | Đã triển khai | 20-40% chi phí Lambda | Gộp lô tin nhắn SQS (ví dụ: 5 hoặc 10 tin nhắn) để gọi ít hàm alert routing Lambda hơn. |
...
| CDO prototype thuần serverless (không hỗ trợ container) | Chi phí cố định cho một tenant thấp hơn, nhưng không đầy đủ cho kịch bản hiện tại | Tránh chi phí ECR image lưu trữ. | Bị từ chối vì kịch bản hiện tại yêu cầu host thời gian chạy AI Engine để tiếp nhận các container image model từ AIOps. |
...
- Chi phí vận hành (phí dịch vụ managed Lambda so với quản trị cụm container tự quản)
...
| DynamoDB | `Cần bằng chứng: dự báo đọc/ghi` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Bộ đệm đọc cho dashboard và metadata. |
| Athena/Glue | `Cần bằng chứng: TB đã quét và số lượng truy vấn catalog` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Xác thực loại bỏ phân vùng và Partition Projection (ADR-014). |
| AI Engine Lambda compute | `Cần bằng chứng: số lượt gọi và số GB-giây` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Thời lượng thực thi hàm Lambda container AI Engine. |
| Hàng đợi SQS & DLQ | `Cần bằng chứng: số lượng tin nhắn` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Hoạt động đệm retry phục vụ cho alert routing. |
...
| VPC Endpoints | `Cần bằng chứng: 7 endpoints × phí hàng giờ` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Truy cập dịch vụ AWS riêng tư. |
| CloudWatch/X-Ray | `Cần bằng chứng: dung lượng log và số custom metric` | `Cần bằng chứng: báo cáo tag Cost Explorer` | `Cần bằng chứng` | Lambda, Step Functions, SQS. |
...
**Insight mong đợi**: Chi phí S3, Athena, DynamoDB và Lambda mở rộng theo số lượng tài khoản và dung lượng dữ liệu. Baseline các VPC endpoint mở rộng dưới dạng chi phí nền tảng cố định dùng chung, trong khi các hàm Lambda container mở rộng theo lưu lượng gọi Lambda và công suất hàng đợi worker.
...
| **Chi phí nền tảng CDO** | `Cần bằng chứng: tổng dự báo CDO` | `Cần bằng chứng` | CDO adapter, lakehouse, dashboard, alert, kiểm toán, baseline VPC endpoint. |
...
| **VPC endpoint hourly cost** | $50.40/tháng cố định | Cảnh báo về sự tăng trưởng baseline không mong đợi | Ngăn ngừa trôi chi phí cố định của mạng riêng tư |
...
- Top 5 nhân tố thúc đẩy chi phí (service breakdown, bao gồm các hàm Lambda compute, SQS, VPC endpoints, CloudWatch, Athena)
- % sử dụng ngân sách
- Chi phí thời gian chạy AI được host tách biệt với chi phí phát triển mô hình của AIOps

*Ghi chú về metric hiệu năng: Các metric hiệu năng (CPU, Memory, database connections, SQS backlogs) được thu thập bởi CloudWatch Metrics, Prometheus, OTel, và X-Ray, và được gửi sang AI Engine như một phần của schema dữ liệu đo lường hiệu năng lai (hybrid telemetry schema). Nếu thiếu các metric này, hệ thống tự động fallback sang chế độ CUR-only, giảm nửa điểm tin cậy và chỉ chạy ở chế độ dry-run/alert-only.*
...
|---|---|---|---|
| **Compute Savings Plans** | Sau 3 tháng baseline | 20-30% trên phần compute baseline ổn định | Áp dụng cho các lần thực thi Lambda (bao gồm cả hàm Lambda container AI Engine và các Lambda adapter CDO khác). |
...
| **Chi phí retry và lưu trữ bộ đệm (cache)** | Biến động | Trung bình | Được giới hạn động bởi các quy tắc giữ lại nhật ký (14 ngày đối với app / 90 ngày đối với audit) và vòng đời bộ đệm (24 giờ cho S3 / 30 ngày cho DynamoDB). |

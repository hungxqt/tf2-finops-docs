...
- **Xác minh (Verification)**: Kiểm tra đường dẫn tệp S3 raw zone: `s3://company-cdo-{account_id}-telemetry/cur/year=2026/month=06/`.
...
### Bước 3 - Gọi API v1/detect của AI Engine qua ALB Nội bộ Riêng tư (Call AI Engine v1/detect API via Private Internal ALB)
...
- **Hành động nội bộ (Internal Action)**: Luồng công việc Step Functions điều phối CDO gọi endpoint `/v1/detect` của ALB nội bộ riêng tư (Application Load Balancer) bằng HTTPS với chữ ký AWS SigV4.
- **Headers yêu cầu (Request Headers)**:
  - `X-Tenant-Id`: Xác định bên thuê (ví dụ: `CDO-01`).
  - `X-Idempotency-Key`: Khóa kết hợp format `{tenant_id}:{billing_period_date}:{batch_type}` (ví dụ: `CDO-01:2026-06-24:daily_batch`) được kiểm tra đối chiếu với bảng DynamoDB `finops-idempotency-{env}` (TTL 24 giờ, conditional write).
  ...
  - `X-Request-Timestamp`: Nhãn thời gian định dạng ISO 8601 (từ chối nếu clock skew lệch > 300 giây).
- **Payload yêu cầu (Request Payload)**:
  ...
  - Các trường sai lệch CUR-CE (bắt buộc khi `telemetry_delay_event = true`): `missing_resources`, `current_ce_cost_gap_usd`, và `comparison_window`.
  - `business_context`: Khối ngữ cảnh bắt buộc chứa phạm vi bao phủ `traffic_volume` (AI Engine tự suy ra `cost_per_request`).
...
- **Xác minh (Verification)**: Kiểm tra nhật ký ALB và nhật ký thực thi Lambda để tìm phản hồi thành công chứa:
  - `success`: `true`
...
### Bước 4 - Xác minh kết quả & Ghi vào Kho lưu trữ có thẩm quyền S3 (Verify Results & Write to S3 Authoritative Store)
...
- **Hành động nội bộ (Internal Action)**: Nền tảng CDO đánh giá phản hồi. Nếu `anomalies_detected` là true, nó ghi trạng thái thực thi và nhật ký kiểm toán vào kho lưu trữ có thẩm quyền S3 (dưới `s3://company-cdo-{account_id}-telemetry/`) có kích hoạt Object Lock, và cập nhật DynamoDB dashboard cache.
...
- **Xác minh & Biện pháp an toàn (Verification & Fail-safes)**:
  - Nếu phát hiện trùng lặp khóa idempotency trong DynamoDB `finops-idempotency-{env}`, AI Engine trả về HTTP `409 Conflict`.
  - Nếu gửi khóa trùng lặp nhưng payload khác nhau, AI Engine trả về HTTP `400 Bad Request` với mã lỗi `ERR_IDEMPOTENCY_MISMATCH`.
  - Nếu Bedrock bị quá thời hạn phản hồi (giới hạn cứng Bedrock API 45 giây, trả về mã lỗi `ERR_LLM_TIMEOUT`) hoặc ALB backend bị dừng, nền tảng CDO lập tức kích hoạt rules engine tĩnh đồng bộ và cảnh báo đội ngũ SRE.
...
### Bước 5 - Nhận Kế hoạch Can thiệp qua ALB Riêng tư (POST /v1/decide) (Get Intervention Plan via Private ALB)
- **Hành động (Action)**: Khi trạng thái hoàn thành, Step Functions gọi endpoint `/v1/decide` của ALB riêng tư để nhận báo cáo phân tích nguyên nhân gốc rễ (RCA) và kế hoạch hành động containment.
- **Tham số yêu cầu (Request Parameters)**: Các headers `X-Correlation-Id` và `X-Tenant-Id` sử dụng chữ ký SigV4.
- **Xác minh (Verification)**: Xác minh rằng AI Engine trả về kế hoạch chứa mã lệnh AWS CLI chính xác (ví dụ: `aws ec2 create-tags` ở chế độ dry-run), payload lệnh hoàn tác rollback tương ứng, và khối cấu hình `rollback_payload.boto3_equivalent` mà CDO lập tức lưu cache vào DynamoDB `finops-rollback-cache` (TTL 90 ngày).
...
### Bước 9 - Xác minh tính hiệu quả của containment qua ALB Riêng tư (POST /v1/verify) (Verify containment effectiveness via Private ALB)
- **Hành động (Action)**: Nền tảng CDO (hoặc Step Functions workflow) gọi `/v1/verify` qua ALB nội bộ riêng tư, truyền vào `correlation_id` và dữ liệu telemetry sau can thiệp.
...
### Bước 10 - Thực thi hoàn tác thủ công/tự động sử dụng bộ đệm rollback của CDO (Execute manual/auto rollback using CDO rollback cache)
...
- **Các tệp nhật ký CUR trong S3 (CUR logs in S3)**: Các tệp thu thập dữ liệu được lưu trữ dưới `s3://company-cdo-{account_id}-telemetry/cur/` xác nhận khả năng tương thích định dạng dữ liệu thô.
- **Xác minh VPC Flow Logs & IAM Telemetry**: Nhật ký cho thấy lưu lượng định tuyến qua Private ALB tới log thực thi Lambda container và lưu lượng qua VPC Endpoint không có lưu lượng thoát ra internet.
- **Nhật ký S3 có thẩm quyền & cache DynamoDB**:
  ...
  - Nhật ký kiểm toán (kiểm toán có thẩm quyền trong S3, cache trong DynamoDB): Bản ghi chứa đầy đủ 14 trường hành động containment, xác minh `correlation_id` khớp với luồng thực thi Step Functions, và chứa một khối liên kết kiểm toán mã hóa tính theo công thức `sha256(current_payload + previous_hash)`. Được lưu trữ trong S3 bucket `company-cdo-{account_id}-telemetry` có kích hoạt Object Lock.
...
- **Tối ưu hóa hosting AI dùng chung (Shared AI hosting optimization)**: Việc triển khai AI Engine trên các container image của AWS Lambda phía sau một Private ALB nội bộ với chữ ký IAM SigV4 giúp tối ưu hóa dung lượng tính toán giữa nhiều nền tảng CDO (loại bỏ chi phí nhàn rỗi) trong khi vẫn giữ cách ly khối lượng công việc thông qua header `X-Tenant-Id` và các chính sách định tuyến của IAM.
- **Tuân thủ đầy đủ (Complete compliance)**: Nhật ký kiểm toán hai lớp (DynamoDB cho tốc độ giao diện người dùng và S3 với Object Lock cho tính không thể sửa đổi trong `s3://company-cdo-{account_id}-telemetry/`) đảm bảo rằng tất cả các hành động tự động và được đề xuất được bảo tồn trong ít nhất 90 ngày, đáp ứng các quy định kiểm toán tài chính. Mỗi mục nhật ký kiểm toán được liên kết mã hóa bằng `sha256(current_payload + previous_hash)` để chống giả mạo.
...
- **Bảo mật hoàn tác được thực thi như thế nào để ngăn chặn các thay đổi tài nguyên trái phép? (How is rollback security enforced to prevent unauthorized resource changes?)**
  - *Phản hồi*: Việc thực thi hoàn tác được xác thực bằng cách sử dụng xác thực Cognito JWT token, xác minh ngữ cảnh bên thuê (tenant context) và được ghi nhật ký vào kho lưu trữ WORM trong S3. CDO backend đọc cấu hình `rollback_payload.boto3_equivalent` đã được lưu cache từ pha decide lưu trong bảng DynamoDB `finops-rollback-cache` và thực thi trực tiếp các cuộc gọi AWS Boto3 tiêu chuẩn, đảm bảo rằng việc khôi phục vẫn được thực thi bình thường ngay cả khi AI Engine ngoại tuyến. Kết quả thực thi sau đó được báo cáo lại cho `/v1/audit/{audit_id}/rollback` qua Private ALB chỉ nhằm đăng ký kiểm toán, sử dụng SQS queue `finops-watch-rollback` cho các thông báo hoàn tất kiểm toán thay vì một hàng đợi lệnh. Để ngăn chặn các cuộc tấn công phát lại (replay attacks) và xử lý độ trễ thanh toán, chúng tôi tách biệt các kiểm tra clock skew: lệch thời gian yêu cầu API > 300 giây sẽ bị từ chối (`ERR_REPLAY_DETECTED`), trong khi độ trễ dữ liệu CUR lên tới 36 giờ được chấp nhận.
- **Ai sở hữu triển khai và vòng đời vận hành Lambda container của AI Engine dùng chung? (Who owns the Lambda container deployment and operational lifecycle of the shared AI Engine?)**
  - *Phản hồi*: CDO sở hữu việc triển khai hạ tầng host (VPC subnets, Private ALB nội bộ, Lambda container function, reserved concurrency, execution roles, và các bảng DynamoDB `finops-idempotency-{env}` và `finops-rollback-cache`) để đáp ứng các yêu cầu mạng riêng tư và bảo mật. AIOps sở hữu logic mô hình AI, logic RCA/khuyến nghị, thực thi rules engine dự phòng cục bộ, tuân thủ hợp đồng API nội bộ, và build container image.
- **Điều gì xảy ra nếu AI Engine gặp lỗi, quá thời gian phản hồi, hoặc nhận được các yêu cầu trùng lặp? (How does the platform handle AI Engine failure, timeout, or duplicate requests?)**
  - *Phản hồi*: Nếu AI Engine phát hiện các khóa idempotency trùng lặp trong DynamoDB `finops-idempotency-{env}`, nó trả về HTTP `409 Conflict` (hoặc `400 ERR_IDEMPOTENCY_MISMATCH` nếu các payload khác nhau). Nếu Bedrock bị hết thời gian chờ (giới hạn cứng 45s của Bedrock API, trả về mã lỗi `ERR_LLM_TIMEOUT`) hoặc Private ALB trả về gateway timeout/down, pipeline CDO sẽ lập tức chuyển sang thực thi rules engine tĩnh và kích hoạt cảnh báo SRE, đảm bảo một trạng thái containment an toàn tuyệt đối. Nếu AI Engine ngoại tuyến trong quá trình thực hiện hành động hoàn tác, CDO backend sẽ tự động thực hiện hoàn tác bằng cấu hình `rollback_payload.boto3_equivalent` đã lưu cache và báo cáo trạng thái thực thi, trả về `audit_recorded = true`.

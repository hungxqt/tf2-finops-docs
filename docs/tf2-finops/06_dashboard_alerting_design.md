...
- **Rollback/Restore Behavior**: Allows engineers to trigger manual rollbacks (representing `/v1/audit/{audit_id}/rollback` semantics), rendered as a **Rollback** button.
  ...
  - *Rollback Execution Logic*: The rollback action is handled directly by the CDO backend. When triggered, the CDO backend reads the cached `rollback_payload.boto3_equivalent` configuration from the authoritative S3 metadata store, executes the rollback actions directly via standard AWS SDK APIs (Boto3/CLI) (ensuring execution works even when the AI Engine itself is offline), and then notifies the AI Engine audit endpoint of the execution status and details.

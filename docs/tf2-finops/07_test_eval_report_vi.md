...
| Tính khả dụng hệ thống | >=99.5% | Tỷ lệ thành công của health check API Gateway | Cần bằng chứng: số liệu thời gian hoạt động của API Gateway |
...
- **Request Format Verification**: Trình kiểm thử gửi các yêu cầu tuân thủ schema phiên bản `telemetry://finops-watch/v3` chứa các header `X-Tenant-Id`, `X-Idempotency-Key` (composite key: `tenant_id:YYYY-MM-DD`), `X-Correlation-Id`, `X-Payload-SHA256`, và `X-Request-Timestamp` tới Private REST API Gateway endpoint sử dụng xác thực IAM SigV4. Kiểm thử xác minh rằng payload yêu cầu là CUR-only (kiểu `RAW_JSON` hoặc `S3_POINTER`) và hoàn toàn không chứa metric hiệu năng CloudWatch.
...
Nếu AI Engine hoàn toàn không thể truy cập được (ví dụ: lỗi HTTP 503, API Gateway gateway/integration timeout, hoặc cạn kiệt concurrency (giới hạn thực thi đồng thời) của hàm Lambda):
...
### 4.6 Kết nối endpoint riêng tư của API Gateway (API Gateway private endpoint connectivity)

- **Ingress qua VPC Endpoint**: Kiểm tra kết nối Private REST API Gateway endpoint từ các private subnet trong VPC.
- **Metric và Xác thực**: Thời gian phản hồi cho health check cơ bản phải dưới 50 mili giây sử dụng chữ ký IAM SigV4.

### 4.7 Tự động mở rộng quy mô và kiểm soát concurrency (Autoscaling and concurrency controls)

- **Giả lập tải đồng thời cao**: Giả lập sự bùng phát lượng cuộc gọi đồng thời tới `/v1/detect`.
- **Reserved Concurrency Guardrail**: Xác minh hàm Lambda tuân thủ giới hạn Reserved Concurrency đã thiết lập để không gây nghẽn cho các dịch vụ quan trọng khác, trả về HTTP `429` (Too Many Requests) hoặc đưa vào hàng đợi SQS một cách phù hợp.
- **Tự động scale-up Provisioned Concurrency (Tùy chọn)**: Xác minh các metric kích hoạt scale-up các thực thi được làm ấm sẵn nếu cấu hình.
...
- **Bước 3 - Gọi API (Step 3 - API Invocation)**: Luồng công việc thu thập dữ liệu trích xuất các bản ghi chi phí, gọi endpoint Private REST API Gateway của AI Engine API, cổng này ghi yêu cầu vào SQS và trả về phản hồi `202 Accepted`.
- **Bước 4 - Thực thi tác vụ (Step 4 - Task Execution)**: Hàm Worker Lambda container được kích hoạt bởi SQS, xử lý các đặc trưng, ghi lại kết quả bất thường/audit vào DynamoDB và lưu trữ lập luận chi tiết.
...
- **API Gateway Resource Policy Isolation**: Xác minh rằng các yêu cầu ingress trực tiếp đến Private REST API Gateway từ bên ngoài VPC endpoint bị chặn bởi Resource Policy và các quy tắc security group.
...
| 2 | Lambda Cold Start Timeout | Việc khởi tạo container Lambda mất nhiều thời gian hơn giới hạn timeout của API Gateway. | Cấu hình SQS để xử lý không đồng bộ (async), tránh bị timeout cổng kết nối. | 3 |

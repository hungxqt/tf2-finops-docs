...
- **Xác thực định dạng yêu cầu (Request Format Verification)**: Trình kiểm thử gửi các yêu cầu tuân thủ schema phiên bản `telemetry://finops-watch/v3` chứa các header `X-Tenant-Id`, `X-Idempotency-Key` (composite key: `tenant_id:YYYY-MM-DD:batch_type`), `X-Correlation-Id`, `X-Payload-SHA256`, and `X-Request-Timestamp` trực tiếp tới hàm Lambda của AI Engine sử dụng quyền gọi IAM (`lambda:InvokeFunction`). Kiểm thử xác minh rằng payload yêu cầu tuân thủ schema dữ liệu đo lường hiệu năng lai (chứa CUR, Cost Explorer và CloudWatch `resource_utilization_metrics`). Nếu các chỉ số CloudWatch bị giả lập là thiếu, kiểm thử xác minh rằng Lambda xử lý việc fallback bằng cách thiết lập `data_confidence = LOW` và bắt buộc chạy ở chế độ dry-run/alert-only.
...
- **Xác thực schema S3 Bucket URI (S3 Bucket URI Schema Validation)**: Xác minh rằng tham số `s3_bucket_uri` được kiểm tra tính hợp lệ dựa trên mẫu biểu thức chính quy (regex) `s3://tf2-cdo{NN}-telemetry-{region}/...json.gz` (trong đó `{NN}` đại diện cho số thứ tự của nhóm và `{region}` đại diện cho mã khu vực AWS, chẳng hạn như `ap-southeast-1`), trả về lỗi xác thực schema cho các URI không đúng định dạng.

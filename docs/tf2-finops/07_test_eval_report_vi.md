...
Việc tuân thủ các giới hạn bắt buộc theo hợp đồng từ `ai-api-contract.md` §6 được đo lường theo chương trình cho đường dẫn thực thi Lambda/SQS trực tiếp:

| Chỉ số SLO hợp đồng | Mục tiêu | Điểm đo lường | Kết quả xác minh |
|---|---|---|---|
| Độ trễ hàng đợi yêu cầu (P99) | < 50 ms | Thời gian thực thi AI Engine Request Lambda (xác thực đầu vào & đẩy vào SQS) | Cần bằng chứng: số liệu đo lường của Lambda |
| Độ trễ truy vấn kết quả (P99) | < 10 ms | Độ trễ truy vấn DynamoDB GET cho `audit_id` (được sử dụng bởi Step Functions poll) | Cần bằng chứng: số liệu đo lường của DynamoDB |
| SLA suy luận LLM | < 30 giây | Thời gian chạy logic suy luận nội bộ của Worker Lambda | Cần bằng chứng: số liệu đo lường của Worker Lambda |
| Tính khả dụng hệ thống | >=99.5% | Tỷ lệ gọi Lambda trực tiếp thành công (không tính lỗi do client bị chặn tần suất) | Cần bằng chứng: số liệu CloudWatch metrics |
| Tỷ lệ lỗi Ingestion | < 0.5% | Số lần chạy lỗi (tin nhắn DLQ / tổng số yêu cầu xử lý CUR) | Cần bằng chứng: số liệu hàng đợi SQS |
...
- **AI Engine Lambda Startup Timeouts / Cold Starts**: Nếu các hàm Lambda container không khởi động được hoặc gặp độ trễ lớn (cold start) trong các sự kiện mở rộng quy mô cao điểm, nền tảng sẽ kích hoạt cảnh báo để thực hiện tối ưu hóa Provisioned Concurrency hoặc đưa yêu cầu trở lại hàng đợi SQS để duy trì SLO về độ trễ.
...
- **Xác thực định dạng yêu cầu (Request Format Verification)**: Trình kiểm thử gửi các yêu cầu tuân thủ schema phiên bản `telemetry://finops-watch/v3` chứa các header `X-Tenant-Id`, `X-Idempotency-Key` (composite key: `tenant_id:YYYY-MM-DD`), `X-Correlation-Id`, `X-Payload-SHA256`, và `X-Request-Timestamp` trực tiếp tới hàm AI Engine Request Lambda sử dụng quyền gọi IAM (`lambda:InvokeFunction`). Kiểm thử xác minh rằng payload yêu cầu tuân thủ schema dữ liệu đo lường hiệu năng lai (chứa CUR, Cost Explorer và CloudWatch `resource_utilization_metrics`). Nếu các chỉ số CloudWatch bị giả lập là thiếu, kiểm thử xác minh Request Lambda xử lý việc fallback bằng cách giảm nửa điểm tin cậy (`confidence *= 0.5`) và bắt buộc chạy ở chế độ dry-run/alert-only.
- **Xác thực định dạng phản hồi (Response Format Verification)**: Kiểm thử xác minh rằng AI Engine Request Lambda trả về payload chứa các tham số bắt buộc: `success`, `status` (`processing`), và `correlation_id`. Nó cũng xác thực rằng payload kết quả cuối cùng được ghi vào DynamoDB/S3 bởi Worker Lambda chứa `status` (`COMPLETED` | `FAILED`), và mảng `anomalies_list` (chứa `anomaly_metadata`, `finance_dashboard_data` và `engineering_dashboard_data`).
...
- **Execution**: Bộ điều phối Step Functions của CDO gọi Request Lambda và tiến hành thăm dò DynamoDB để lấy kết quả.
- **Verification**: Nếu Worker Lambda không ghi được kết quả trong giới hạn thời gian chờ của bước thực thi Step Functions, nền tảng sẽ ghi lại cảnh báo hết thời gian chờ, đánh dấu trạng thái thất bại và gửi cảnh báo tới quản trị viên.

### 4.3 Cơ chế dự phòng khi AI không khả dụng (Unavailable-AI fallback)

Nếu AI Engine hoàn toàn không thể truy cập được (ví dụ: lỗi gọi Lambda, lỗi đẩy hàng đợi SQS, hoặc cạn kiệt concurrency (giới hạn thực thi đồng thời) của hàm Lambda):
...
- **Giả lập gián đoạn thực thi**: Giả lập lỗi hoặc timeout trong quá trình thực thi hàm Lambda worker container.
- **Độ tin cậy của SQS**: Xác minh SQS giữ tin nhắn, tăng bộ đếm số lần nhận (receive count), và kích hoạt lại hàm worker Lambda sau khi visibility timeout hết hạn.
- **Redrive sang DLQ**: Giả lập việc vượt quá số lần thử lại tối đa và xác minh tin nhắn được đưa an toàn sang Dead Letter Queue (DLQ) để phân tích lỗi, đồng thời ghi bản ghi kiểm toán failure.

### 4.6 Dung lượng đệm hàng SQS và kiểm soát concurrency (SQS message buffering capacity and concurrency controls)

- **SQS Queue Buffering**: Xác minh rằng dưới tải đỉnh, Request Lambda đẩy thành công các yêu cầu xử lý CUR vào hàng SQS mà không bị mất tin nhắn.
- **Concurrency Rate Limits**: Xác thực rằng concurrency thực thi của Worker Lambda tự động mở rộng theo độ sâu hàng đợi SQS, tuân thủ giới hạn Reserved Concurrency để tránh gây quá tải hạ tầng phía sau.

### 4.7 Tự động mở rộng quy mô và kiểm soát concurrency (Autoscaling and concurrency controls)

- **Giả lập tải đồng thời cao**: Giả lập sự bùng phát lượng cuộc gọi đồng thời trực tiếp tới Request Lambda.
- **Reserved Concurrency Guardrail**: Xác minh các hàm Request và Worker Lambda tuân thủ giới hạn Reserved Concurrency đã thiết lập để không gây nghẽn cho các dịch vụ quan trọng khác, trả về lỗi gọi hàm bị throttling hoặc đưa vào hàng đợi SQS một cách phù hợp.
- **Kiểm thử Provisioned Concurrency (Tùy chọn)**: Nếu được kích hoạt, xác minh các môi trường thực thi Lambda được làm ấm sẵn được phân bổ và định tuyến yêu cầu thành công mà không gặp độ trễ cold-start.

### 4.8 Kiểm thử kết quả Thăm dò và Phân trang (Result Polling & Pagination Tests)

- **Xác thực Phản hồi Polling**: Khung kiểm thử truy vấn bảng kết quả/thực thi DynamoDB theo `correlation_id` (mô phỏng ngữ nghĩa logic của cuộc gọi `GET /v1/status/{id}`). Nó xác minh các trạng thái phản hồi của hợp đồng (`COMPLETED` vs. `PROCESSING` vs. `FAILED`) và xác thực cấu trúc của đối tượng `anomalies_list` được lưu trữ.
- **Xác thực Phân trang**: Dưới tải kiểm thử tạo ra nhiều bất thường, khung kiểm thử truy vấn kết quả với giới hạn kích thước bản ghi để xác minh `next_token` được tạo. Sau đó, nó truy vấn các trang tiếp theo bằng token này, xác nhận thứ tự index kết quả chính xác.

### 4.9 Kiểm thử Hành động Gia hạn & Hoàn tác (Extend & Rollback Semantics Tests)

- **Kiểm thử Lập Kế hoạch Can thiệp (Decide Action Plan Test)**: Mô phỏng việc bộ điều phối gọi AI Engine Worker Lambda (đại diện cho ngữ nghĩa `POST /v1/decide`) để lấy kế hoạch hành động. Kiểm thử xác minh kế hoạch trả về chứa `dry_run_mode`, `applied_payload` (với `aws_cli_command`) và `rollback_payload` (với `aws_cli_rollback_command`), cùng với cấu hình phân quyền Cognito và tiêu đề `X-Containment-Status: LOCKED` nếu tỷ lệ rollback của tenant vi phạm ngưỡng.
- **Kiểm thử Hoàn tác (Rollback Action Test)**: Mô phỏng việc kích hoạt hoàn tác thủ công trên bảng điều khiển bằng cách gọi endpoint rollback (đại diện cho ngữ nghĩa `POST /v1/audit/{audit_id}/rollback`). Kiểm thử xác minh rằng:
  - Lambda kiểm tra xác thực, email `rolled_back_by` và khớp `X-Tenant-Id`.
  - Nó khởi tạo rollback thành công, trả về `rollback_initiated = true`, cập nhật bộ đếm số ca FP và tính toán lại `new_error_budget_burned_pct`.
  - Nếu tỷ lệ ngân sách lỗi bị hao hụt vượt quá 1%, kiểm thử xác minh tenant chuyển sang `LOCKED_MODE` (trả về cờ `containment_locked = true`).
...
- **Trùng Khóa Idempotency (Ngữ nghĩa `409` & `400`)**:
  - **Lượt gọi đang chạy (Ngữ nghĩa `409 Conflict`)**: Gọi Request Lambda với khóa idempotency đang hoạt động sẽ trả về cấu trúc phản hồi xung đột (đại diện cho ngữ nghĩa HTTP `409`) và ngăn thực thi trùng lặp.
...
- **Dự phòng Timeout Bedrock**: Khi cuộc gọi Bedrock giả lập vượt quá timeout cứng 45 giây, hệ thống xác nhận AI Engine Worker Lambda tự hủy tác vụ và ghi trạng thái `failed` cùng mã lỗi `ERR_LLM_TIMEOUT` vào DB. Cơ chế polling của CDO bắt lỗi thất bại này và chuyển ngay sang Rules Engine tĩnh dự phòng.
...
- **Ghi nhật ký Kiểm toán cho Sự kiện Auth (Audit Logging for Auth Events)**: Xác nhận rằng các sự kiện đăng nhập/đăng xuất, lỗi xác thực mã token và các nỗ lực thực hiện hành động trái phép (ví dụ: người dùng Finance cố gắng khôi phục) đều được ghi nhận vào nhật ký kiểm toán trên DynamoDB và S3 (ví dụ: ghi lại trạng thái `auth_success`, `auth_failure`, hoặc `unauthorized_action_blocked`).
...
- **Bước 3 - Gọi Lambda (Step 3 - Lambda Invocation)**: Luồng công việc thu thập dữ liệu trích xuất các bản ghi chi phí, gọi trực tiếp hàm AI Engine Request Lambda, hàm này xác thực dữ liệu, ghi yêu cầu vào SQS và trả về phản hồi đang xử lý.
- **Bước 4 - Thực thi tác vụ (Step 4 - Task Execution)**: Hàm Worker Lambda container được kích hoạt bởi SQS, xử lý dữ liệu chi phí, ghi lại kết quả bất thường/audit vào DynamoDB và lưu trữ lập luận chi tiết vào S3.
...
- **Bước 8 - Giả lập hoàn tác (Step 8 - Rollback Simulation)**: Quản trị viên nhấp vào nút "Rollback" trên bảng điều khiển CDO, kích hoạt endpoint rollback (đại diện cho ngữ nghĩa `POST /v1/audit/{audit_id}/rollback`), bắt đầu khôi phục lại tag của tài nguyên chéo tài khoản và cập nhật số liệu đo lường về ngân sách lỗi.

...
Việc tuân thủ các giới hạn bắt buộc theo hợp đồng từ `ai-api-contract.md` §6 được đo lường theo chương trình cho đường dẫn thực thi Lambda trực tiếp:

| Chỉ số SLO hợp đồng | Mục tiêu | Điểm đo lường | Kết quả xác minh |
|---|---|---|---|
| Độ trễ yêu cầu (P99) | < 300 ms | Thời gian thực thi AI Engine Lambda (xác thực đầu vào & chạy phát hiện đồng bộ) | Cần bằng chứng: số liệu đo lường của Lambda |
...
- **State Check**: Step Functions kiểm tra xem kho lưu trữ có thẩm quyền S3 (authoritative S3 store) đã có bản ghi cho `idempotency_key` hay chưa (được định dạng là `AccountID:DateWindow`).
- **Execution Bypass**: Lượt chạy thứ hai được bỏ qua một cách thành công, và không có đối tượng S3 hay tin nhắn cảnh báo trùng lặp nào được tạo ra.
...
- **Xác thực định dạng yêu cầu (Request Format Verification)**: Trình kiểm thử gửi các yêu cầu tuân thủ schema phiên bản `telemetry://finops-watch/v3` chứa các header `X-Tenant-Id`, `X-Idempotency-Key` (composite key: `tenant_id:YYYY-MM-DD:batch_type`), `X-Correlation-Id`, `X-Payload-SHA256`, và `X-Request-Timestamp` trực tiếp tới hàm Lambda của AI Engine sử dụng quyền gọi IAM (`lambda:InvokeFunction`). Kiểm thử xác minh rằng payload yêu cầu tuân thủ schema dữ liệu đo lường hiệu năng lai (chứa CUR, Cost Explorer và CloudWatch `resource_utilization_metrics`). Nếu các chỉ số CloudWatch bị giả lập là thiếu, kiểm thử xác minh rằng Lambda xử lý việc fallback bằng cách giảm nửa điểm tin cậy (`confidence *= 0.5`) và bắt buộc chạy ở chế độ dry-run/alert-only.
- **Xác thực định dạng phản hồi (Response Format Verification)**: Kiểm thử xác minh rằng Lambda của AI Engine trả về payload đồng bộ chứa các tham số bắt buộc: `success`, `correlation_id`, `anomalies_detected`, và `anomalies_list` (chứa `anomaly_id`, `anomaly_type`, `severity`, `confidence_score`, `resource_id`, `environment`, `responsible_team`, `unblended_cost_24h_usd`, `cost_ratio_to_7d_avg`, `ai_model_used`, `alert_routing`).
...
- **Execution**: Bộ điều phối Step Functions của CDO gọi hàm Lambda của AI Engine một cách đồng bộ.
- **Verification**: Nếu Lambda của AI Engine không trả về kết quả trong giới hạn thời gian chờ của bước thực thi Step Functions, nền tảng sẽ ghi lại cảnh báo hết thời gian chờ và gửi cảnh báo tới quản trị viên.

### 4.3 Cơ chế dự phòng khi AI không khả dụng (Unavailable-AI fallback)

Nếu AI Engine hoàn toàn không thể truy cập được (ví dụ: lỗi gọi Lambda hoặc cạn kiệt concurrency (giới hạn thực thi đồng thời) của hàm Lambda):
...
- **Kiểm thử Kế hoạch Hành động Decide (Decide Action Plan Test)**: Mô phỏng việc bộ điều phối gọi Lambda của AI Engine (đại diện cho ngữ nghĩa `POST /v1/decide`) để lấy kế hoạch can thiệp. Kiểm thử xác minh kế hoạch trả về chứa `dry_run_mode`, `applied_payload` (với `aws_cli_command`) và `rollback_payload` (với `aws_cli_rollback_command`), cùng với các quy tắc truy cập dựa trên nhóm Cognito và tiêu đề `X-Containment-Status: LOCKED` nếu tỷ lệ rollback của tenant vi phạm ngưỡng.
- **Kiểm thử Hành động Hoàn tác (Rollback Action Test)**: Mô phỏng việc kích hoạt hoàn tác thủ công trên bảng điều khiển bằng cách gọi endpoint rollback (đại diện cho ngữ nghĩa `POST /v1/audit/{audit_id}/rollback`). Kiểm thử xác minh rằng:
  ...
  - Nó khởi tạo rollback thành công, trả về `rollback_initiated = true`, cập nhật bộ đếm số lỗi sai (false positive count) và tính toán lại tỷ lệ hao hụt ngân sách lỗi (`new_error_budget_burned_pct`).
...
- **Bước 3 - Gọi Lambda (Step 3 - Lambda Invocation)**: Luồng công việc thu thập dữ liệu trích xuất các bản ghi chi phí và gọi trực tiếp hàm Lambda của AI Engine một cách đồng bộ, hàm này trả về chỉ thị phát hiện thành công và danh sách các bất thường.

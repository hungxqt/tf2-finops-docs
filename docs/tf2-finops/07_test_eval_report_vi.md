...
- **Xác thực định dạng yêu cầu (Request Format Verification)**: Trình kiểm thử gửi các yêu cầu tuân thủ schema phiên bản `telemetry://finops-watch/v3` chứa các header `X-Tenant-Id`, `X-Idempotency-Key` (composite key: `tenant_id:YYYY-MM-DD`), `X-Correlation-Id`, `X-Payload-SHA256`, và `X-Request-Timestamp` trực tiếp tới hàm AI Engine Request Lambda sử dụng quyền gọi IAM (`lambda:InvokeFunction`). Kiểm thử xác minh rằng payload yêu cầu là CUR-only (kiểu `RAW_JSON` hoặc `S3_POINTER`) và hoàn toàn không chứa metric hiệu năng CloudWatch.
- **Xác thực định dạng phản hồi (Response Format Verification)**: Kiểm thử xác minh rằng AI Engine Request Lambda trả về payload chứa các tham số bắt buộc: `audit_id`, `status` (`processing`), và `retry_after_seconds`. Nó cũng xác thực rằng payload kết quả cuối cùng được ghi vào DynamoDB/S3 bởi Worker Lambda chứa `status` (`completed` | `failed`), mảng `anomalies_list` (chứa `anomaly_metadata`, `finance_dashboard_data` và `engineering_dashboard_data`) và cấu trúc phân trang `pagination` (`next_token` và `limit`).
...
- **Xác thực Phản hồi Polling**: Khung kiểm thử truy vấn bảng kết quả/thực thi DynamoDB theo `audit_id` (mô phỏng ngữ nghĩa logic của cuộc gọi `GET /v1/detect/result/{audit_id}`). Nó xác minh các trạng thái phản hồi của hợp đồng (`completed` vs. `processing` vs. `failed`) và xác thực cấu trúc của đối tượng `anomalies_list` được lưu trữ.
...
- **Kiểm thử Gia hạn (Extend/Snooze Action Test)**: Mô phỏng hành động của kỹ sư bằng cách kích hoạt Lambda xử lý gia hạn (đại diện cho ngữ nghĩa logic `/v1/action/extend`) với `extend_seconds` và `reason`. Kiểm thử xác minh Lambda cập nhật bộ đếm thời gian countdown của tài nguyên trong DynamoDB và trả về `new_expiration_time`.
- **Kiểm thử Hoàn tác (Rollback Action Test)**: Mô phỏng việc nhấp vào "Revert" trên bảng điều khiển bằng cách gọi Lambda xử lý rollback (đại diện cho ngữ nghĩa logic `/v1/action/rollback`). Kiểm thử xác minh rằng:
  - Lambda kiểm tra xác thực và khớp `X-Tenant-Id`.
...
- **Kiểm thử Quyền chỉ đọc của Nhóm Finance (Finance Group Read-Only Access Test)**: Xác thực rằng người dùng thuộc nhóm người dùng Cognito `finops-finance-readonly` được xác thực thành công nhưng bị giới hạn ở các chế độ xem chỉ đọc trên dashboard. Các yêu cầu thực hiện hành động ngăn chặn (Kéo dài hoặc Khôi phục) qua các yêu cầu POST gửi tới `/v1/action/extend` hoặc `/v1/action/rollback` đều bị từ chối với lỗi HTTP `403 Forbidden` / `ERR_INSUFFICIENT_PERMISSIONS`.
- **Kiểm thử Phân quyền Hành động của Nhóm Engineering (Engineering Group Action Authorization Test)**: Xác minh rằng người dùng trong các nhóm `finops-engineering-operator` và `finops-cdo-admin` có thể kích hoạt thành công các hành động Kéo dài/Khôi phục (Extend/Rollback) trên dashboard, xác nhận các mã JWT chứa đúng các thông tin nhóm (group claims) khi chuyển tiếp đến các endpoint API hành động.
...
- **Bước 8 - Giả lập hoàn tác (Step 8 - Rollback Simulation)**: Quản trị viên nhấp vào nút "Revert" trên bảng điều khiển CDO, kích hoạt Lambda xử lý hoàn tác để thực thi các bước rollback được xác định trong bản ghi kiểm toán, đưa các tag trở lại trạng thái baseline ban đầu.

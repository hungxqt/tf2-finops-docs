...
- **Glue Crawler & Athena Views**: Các kiểm thử xác minh rằng Glue Crawler biên mục thành công cấu trúc phân vùng raw trên S3 và các truy vấn Athena có thể tổng hợp các chỉ số chi phí theo dịch vụ, khu vực, tài khoản và các tag tài nguyên mà không gặp lỗi cú pháp.
...
- **Request Format Verification**: Trình kiểm thử gửi các yêu cầu chứa `correlation_id`, `anomaly_id` và `cost_window` để xác minh rằng chúng phù hợp với schema.
- **Response Format Verification**: Kiểm thử xác minh rằng AI Engine trả về phản hồi chứa các tham số bắt buộc: `confidence_score` (từ 0.00 đến 1.00) và `explanation` (chuỗi không rỗng).
...
Containment engine phải tạo một mục nhật ký kiểm toán cho mỗi lần thử hành động. Kiểm thử này xác minh rằng schema JSON được ghi chứa đầy đủ 14 trường bắt buộc:
...
4. `idempotency_key`: Khóa ngăn chặn thực thi kép.

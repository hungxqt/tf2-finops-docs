...
| 11 | Lambda Yêu cầu AI Engine (AI Engine Request Lambda) | AWS Lambda | Xác thực payload đầu vào và đưa yêu cầu vào SQS (đại diện cho ngữ nghĩa hợp đồng POST /v1/detect). |
...
| 20 | DynamoDB Trạng thái Chạy và Kiểm toán (DynamoDB Run State and Audit) | Amazon DynamoDB | Lưu trữ khóa chạy (run locks), tính không thay đổi khi chạy lại (idempotency), chỉ mục kiểm toán (audit index) và dữ liệu cụ thể hóa của dashboard. |
...
| 26 | AI Engine Worker Lambda | AWS Lambda | Tiêu thụ các yêu cầu từ SQS, chạy container mô hình và ghi kết quả vào S3 và DynamoDB. |
...
| 28 | Hàng đợi SQS/DLQ của AI Engine | Amazon SQS | Quản lý các hàng đợi tách rời: hàng đợi yêu cầu và hàng đợi thư rác (DLQ). |
| 29 | Các kho lưu trữ DynamoDB của AI Engine | Amazon DynamoDB | Các bảng trạng thái bền vững cho kết quả thực thi, tính không lặp lại (idempotency) và bất thường. Step Functions trực tiếp truy vấn kho lưu trữ này để lấy kết quả. |
...
Lambda thu nhận kéo dữ liệu Cost Explorer và sao chép hoặc đăng ký các tệp CUR.

### Mục đích (Purpose)

Nó chuyển đổi các nguồn thanh toán bên ngoài thành đầu vào nền tảng thô (raw platform inputs) được lưu trữ dưới lakehouse của CDO.

### Đầu vào (Input)

- Danh sách tài khoản và chi tiết giả lập vai trò (role-assumption details).
- Tên và tiền tố bucket CUR (CUR bucket names and prefixes).
- Khung thời gian truy vấn Cost Explorer.
- Bucket S3 và tiền tố thô (raw prefix).
...
- Các tệp JSON Cost Explorer thô.
...
Xác thực các payload đầu vào và đưa các yêu cầu vào SQS (đại diện cho ngữ nghĩa hợp đồng POST `/v1/detect` ở cấp độ hợp đồng).

### Mục đích (Purpose)

Đóng vai trò là điểm bắt đầu cho luồng phát hiện bất thường AI. Nó xác thực các schema yêu cầu đầu vào, kiểm tra xung đột tính không lặp lại (idempotency), và đưa yêu cầu vào hàng đợi SQS để xử lý không đồng bộ phía dưới.

### Đầu vào (Input)

- Payload khung thời gian chi phí đã chuẩn hóa.
- ID lượt chạy và khóa không thay đổi khi chạy lại.
- Phạm vi tài khoản.
- Phiên bản hợp đồng.
- Tham chiếu bí mật để ký xác thực tính toàn vẹn payload.
- URL hàng đợi SQS phía dưới.

### Đầu ra (Output)

- Trạng thái thành công (tương đương cấp độ hợp đồng với mã HTTP `202 Accepted`) chứa:
  - `audit_id` (UUID theo dõi)
  - `status`: `"processing"`
  - `retry_after_seconds`: `30`
- Tin nhắn được đưa thành công vào SQS.
...
Tiêu thụ các yêu cầu từ SQS, chạy container mô hình AI, và ghi kết quả vào S3 và DynamoDB.

### Mục đích (Purpose)

Thực hiện suy luận mô hình và phân tích bất thường bên trong một hàm Lambda được khởi tạo từ hình ảnh container do AIOps cung cấp. Nó được kích hoạt bởi các tin nhắn SQS và sử dụng mức concurrency dành riêng để kiểm soát vùng ảnh hưởng (blast radius) và các giới hạn nghẽn (throttle limits).

### Đầu vào (Input)

- Các tin nhắn SQS chứa các mục tiêu phát hiện bất thường.
- Hash hình ảnh ECR không thể thay đổi (digest pinning).
- ARN bảng DynamoDB và bucket S3 phía dưới.
- ARN vai trò thực thi Lambda.
- Cấu hình mức concurrency dành riêng (reserved concurrency).

### Đầu ra (Output)

- Kết quả phát hiện bất thường và giải thích được ghi trực tiếp vào DynamoDB của AI Engine.
...
## 28. Hàng đợi SQS/DLQ của AI Engine
...
Xử lý các yêu cầu phát hiện đầu vào dưới dạng tin nhắn SQS để cho phép Lambda Yêu cầu (Request Lambda) phản hồi nhanh chóng, và cho phép Lambda Worker tiêu thụ tin nhắn ở tốc độ được kiểm soát, định tuyến các tin nhắn bị lỗi liên tục (poison messages) đến DLQ sau 3 lần thử lại.
...
## 29. Các kho lưu trữ DynamoDB của AI Engine
...
- Các lệnh ghi từ Lambda Worker của AI Engine, các Lambda nền tảng CDO.

### Đầu ra (Output)

- Trạng thái và kết quả với độ trễ thấp được thăm dò trực tiếp bởi Step Functions và dashboard S3 + CloudFront.

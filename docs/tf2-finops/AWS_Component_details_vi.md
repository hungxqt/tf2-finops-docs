...
| 11 | Lambda Client Hợp đồng AI (AI Contract Client Lambda) | AWS Lambda | Gọi hợp đồng AI Engine bên ngoài và đóng lỗi an toàn (fails closed) nếu phản hồi không hợp lệ. |
...
Lambda Client Hợp đồng AI gọi điểm cuối AI Engine dùng chung của Task Force.
...
- Khung thời gian chi phí đã chuẩn hóa (normalized cost window).
- ID lượt chạy (Run ID) và khóa không thay đổi khi chạy lại (idempotency key).
- Phạm vi tài khoản.
- Phiên bản hợp đồng (contract version).
- Tham chiếu bí mật cho endpoint AI.
- Tham chiếu bí mật cho khóa ký hợp đồng.
- Thiết lập hết giờ (timeout).
- Schema phản hồi bắt buộc.

### Đầu ra (Output)

- Payload quyết định AI đã được xác thực.
- Phiên bản mô hình (model version) do AIOps trả về.
- ID bất thường (Anomaly ID).
- Độ tin cậy (confidence) và độ nghiêm trọng (severity).
- Chi tiêu kỳ vọng (expected spend), chi tiêu thực tế (actual spend), và mức chênh lệch (delta).
- Giải thích (explanation) và định tuyến được đề xuất (recommended route).
- Chế độ ngăn chặn được đề xuất (recommended containment mode).
- URI bằng chứng (evidence URI).
- Đóng lỗi an toàn (fail-closed error) khi endpoint AI không khả dụng hoặc schema phản hồi không hợp lệ.
...
Một dashboard web nội bộ nhẹ nhàng được lưu trữ dưới dạng các static assets trong Amazon S3 và phân phối qua Amazon CloudFront. Giao diện này đọc các tóm tắt đã được tính toán trước, dễ đọc cho bộ phận tài chính từ các đối tượng JSON trên S3 hoặc các bản ghi DynamoDB được tạo ra từ quy trình thu nhận được lên lịch hàng ngày. Athena duy trì hoạt động ngầm để tạo các tóm tắt chuẩn hóa; người dùng Tài chính không bao giờ phải viết SQL.
...
| Hành vi lỗi | Đóng lỗi an toàn (fail closed), chặn ngăn chặn, cảnh báo cho kỹ sư vận hành và ghi lại trạng thái hợp đồng bị lỗi |

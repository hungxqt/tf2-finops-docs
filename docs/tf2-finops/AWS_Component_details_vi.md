...
| 20 | Kho lưu trữ Idempotency DynamoDB (DynamoDB Idempotency Store) | Amazon DynamoDB | Quản lý việc kiểm tra tính không thay đổi khi chạy lại của scheduled run (`finops-idempotency-{env}`) với các ghi chép có điều kiện và TTL 24 giờ. |
...
| 26 | Thực thi Lambda AI Engine (AI Engine Lambda Execution) | AWS Lambda | Chạy container mô hình phía sau một Private ALB nội bộ để trả về các bất thường. |
...
| 29 | Bộ đệm Hoàn tác DynamoDB (DynamoDB Rollback Cache) | Amazon DynamoDB | Cập nhật bộ đệm cho khối cấu hình `rollback_payload.boto3_equivalent` (`finops-rollback-cache`) với TTL 90 ngày. |
...
| 32 | Private ALB của AI Engine (AI Engine Private ALB) | Amazon ELB | Cung cấp giao diện HTTPS nội bộ riêng tư (/v1/*) với xác thực SigV4 để định tuyến cuộc gọi đến Lambda container. |
...
- Tên và tiền tố bucket CUR (tiêu chuẩn đặt tên bucket: `s3://company-cdo-{account_id}-telemetry/`).
...
- Bucket S3 và tiền tố thô (tuân thủ tiêu chuẩn đặt tên bucket chuẩn: `s3://company-cdo-{account_id}-telemetry/raw/`).
...
- ID lượt chạy và khóa không thay đổi khi chạy lại (được cache trong bảng DynamoDB `finops-idempotency-{env}`).
- Phạm vi tài khoản.
- Phiên bản hợp đồng.
- Các chỉ số hiệu năng CloudWatch (bao gồm cả mảng `cpu_utilization_hourly` thô mỗi 24 giờ).
...
- Cấu hình mã hóa KMS.
- Tiêu chuẩn đặt tên bucket: đặt tên bucket chuẩn `s3://company-cdo-{account_id}-telemetry/raw/`.
...
- Tiêu chuẩn đặt tên bucket: đặt tên bucket chuẩn `s3://company-cdo-{account_id}-telemetry/curated/`.
...
- Tiêu chuẩn đặt tên bucket: đặt tên bucket chuẩn `s3://company-cdo-{account_id}-telemetry/audit/` và `s3://company-cdo-{account_id}-telemetry/telemetry/`.
...
DynamoDB lưu trữ các bản ghi tính không thay đổi khi chạy lại (idempotency) của scheduled run (`finops-idempotency-{env}`).

### Mục đích (Purpose)

Nó cung cấp các kiểm tra idempotency nóng với độ trễ thấp sử dụng cơ chế ghi có điều kiện để tránh các lượt chạy trùng lặp cho cùng một tài khoản và khung thời gian thanh toán.

### Đầu vào (Input)

- ID lượt chạy (Run ID).
- Khóa không thay đổi khi chạy lại (idempotency key, định dạng: `{tenant_id}:{billing_period_date}:{batch_type}`).
- Tài khoản và khung thời gian thanh toán (billing window).
- Trạng thái lượt chạy (state status).
- Thời gian sống hết hạn (TTL - 24h).

### Đầu ra (Output)

- Kết quả khóa lượt chạy (run lock result).
- Bản ghi trạng thái lượt chạy.
- Phát hiện chạy trùng lặp và trạng thái khóa.
...
- Dữ liệu chi phí thô và các chỉ số sử dụng CloudWatch từ S3 (bao gồm mảng `cpu_utilization_hourly` và cờ `telemetry_delay_event`; tiêu chuẩn đặt tên bucket: `s3://company-cdo-{account_id}-telemetry/`).
...
Kho lưu trữ bộ đệm cho các payload thực thi hoàn tác (`finops-rollback-cache`).

### Mục đích (Purpose)

Cung cấp cơ chế cache độ bền cao, độ trễ thấp cho khối cấu hình `rollback_payload.boto3_equivalent` ngay sau pha `/v1/decide` để thực thi độc lập các cuộc gọi hoàn tác offline Boto3.

### Đầu vào (Input)

- Khối cấu hình `rollback_payload.boto3_equivalent` từ pha decide.
- ID bất thường (Anomaly ID).
- Thời gian sống (TTL - 90 ngày).

### Đầu ra (Output)

- Payload hoàn tác đã được cache để CDO containment worker truy xuất nhằm thực thi Boto3.
...
## 32. Private ALB của AI Engine (AI Engine Private ALB)

### Vai trò (Role)

Cung cấp giao diện HTTPS nội bộ riêng tư (`/v1/*`) với xác thực SigV4.

### Purpose

Cho phép gọi và thực thi an sau, nằm trong giới hạn VPC, đối với hàm container AI Engine đằng sau một bộ cân bằng tải riêng tư, đồng thời áp dụng kiểm tra tính toàn vẹn (`X-Payload-SHA256`) và kiểm soát thời gian chờ yêu cầu.

### Đầu vào (Input)

- Các yêu cầu HTTPS từ Step Functions chạy trên các subnet riêng tư của VPC.
- Tiêu đề AWS Signature Version 4 (SigV4).
- Các tiêu đề hợp đồng bắt buộc (`X-Tenant-Id`, `X-Idempotency-Key`, `X-Correlation-Id`, `X-Payload-SHA256`, `X-Request-Timestamp`, và `X-Dry-Run-Mode`).

### Đầu ra (Output)

- Các yêu cầu kích hoạt hàm Lambda được định tuyến an toàn trong VPC.
- Các phản hồi HTTPS an toàn được trả lại cho Step Functions.

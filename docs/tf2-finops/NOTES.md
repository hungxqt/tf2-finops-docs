...
1. **CDO Platform Observability**: Actual measurements must capture the CDO-owned adapters (Lambda execution times, SQS/DLQ queuing latency, private ALB routing volumes, and Athena query sizes) and the operational hosting platform parameters.
...
3. **Hybrid Telemetry Scope**: In addition to cost data (CUR and Cost Explorer API fallback), the detection loop leverages CloudWatch infrastructure metrics. Specifically, the CDO sends a `cpu_utilization_hourly` array containing hourly CPU utilization metrics. If CloudWatch utilization metrics are missing, the platform automatically falls back to CUR-only mode, setting `data_confidence = LOW` and locking containment actions to dry-run/alert-only.
4. **Daily Ingestion & Cost Explorer Fallback**: Under normal scheduled execution, the CDO platform executes a **CUR-first daily anomaly detection** workflow. If the S3 CUR data transmission is delayed (which is flagged with `telemetry_delay_event = true` in the API contract), the platform automatically falls back to querying the AWS Cost Explorer API for cost metrics. In this fallback scenario, the AI Engine returns `data_confidence = LOW`, and the CDO platform overrides the containment mode to a strict **dry-run/alert-only containment mode** to avoid incorrect automated actions based on lagged data.

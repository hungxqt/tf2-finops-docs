...
* EKS cluster, ECR, Internal ALB và workload chạy trên Kubernetes.
...
graph LR
    ...
    ECR --> EKS[EKS Workloads]
    AWS --> Smoke[Smoke Test]
    EKS --> Smoke
```

*Caption: GitHub Actions kiểm tra thay đổi hạ tầng, tạo Terraform plan, triển khai các tài nguyên AWS, build/push image lên ECR, cập nhật workload trên EKS và chạy smoke test để xác minh hệ thống hoạt động đúng.*
...
* Validate Kubernetes/Helm manifest nếu có thay đổi workload trên EKS.
...
| --------------- | ------------------------------------- | ----------------------------------------------- |
| Validate        | Terraform, Helm, scripts              | Phát hiện lỗi trước khi triển khai.             |
...
| Deploy workload | EKS                                   | Cập nhật AI Engine API/worker sau Internal ALB. |
| Smoke test      | Step Functions, Lambda, EKS, DynamoDB | Kiểm tra workflow FinOps sau triển khai.        |
...
5. Gọi AI Engine chạy trên EKS thông qua Internal ALB.
...
Một lần triển khai chỉ được xem là hợp lệ khi toàn bộ bước validate pass, Terraform apply thành công, workload trên EKS ở trạng thái healthy và smoke test xác nhận được luồng chính: ingest dữ liệu, gọi AI Engine, routing alert, containment dry-run và ghi audit log.

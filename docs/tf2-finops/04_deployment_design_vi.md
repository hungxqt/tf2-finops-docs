...
│   │   ├── dynamodb/             # Read cache cho dashboard và metadata
...
### 1.4 Tiêu chuẩn khởi tạo S3 Bucket (S3 Bucket Creation Standards)

Trong quá trình thiết lập cấu hình Terraform, module `s3-lakehouse` thực thi tiêu chuẩn `tf2-cdo{NN}-telemetry-{region}` cho việc tạo S3 bucket. Mọi định nghĩa tài nguyên bucket bắt buộc phải tuân theo quy tắc đặt tên này, sử dụng các biến cho số hiệu CDO tenant `{NN}` và vùng triển khai AWS `{region}` nhằm ngăn chặn các không gian tên S3 không đạt chuẩn.
...
* Hàm AI Engine Lambda container, ECR, SQS queues cho alert, và các role execution Lambda.
...
| Deploy workload | Lambda (AI Engine)                             | Triển khai hàm Lambda container bằng cách xuất bản version và alias. |
| Smoke test      | Step Functions, Lambda, DynamoDB               | Kiểm tra workflow FinOps sau triển khai.                                   |
...
3. Xác nhận dữ liệu thô (raw) và tinh chế (curated) được ghi vào các S3 bucket tuân theo tiêu chuẩn đặt tên 'tf2-cdo{NN}-telemetry-{region}'.
4. Kiểm tra Glue/Athena có thể query dữ liệu curated.
5. Gọi trực tiếp hàm AI Engine Lambda sử dụng xác thực IAM:
   - Xác thực rằng endpoint '/v1/detect' trả về giá trị 'data_confidence' (LOW/MEDIUM/HIGH).
   - Xác thực rằng endpoint '/v1/decide' trả về 'rollback_payload.boto3_equivalent', dữ liệu này sẽ được CDO cache lại.
6. Xác thực rằng CDO có thể chạy thành công một hoạt động rollback từ cache cục bộ trong khi AI Engine giả lập ở trạng thái offline.
7. Xác thực hàm AI Engine Lambda trả về phản hồi đồng bộ 200 OK với chỉ thị thành công.
```

Lưu ý: Việc xác minh luồng chạy (kiểm tra payload alert routing, kiểm tra trạng thái dry-run của containment, và kiểm toán ghi S3) được tách ra thành một bộ End-to-End (E2E) Test chạy riêng định kỳ để không block pipeline CI/CD cơ bản hoặc lãng phí thời gian runner.

Một lần triển khai chỉ được xem là hợp lệ khi toàn bộ bước validate pass, Terraform apply thành công, các hàm Lambda ở trạng thái healthy và smoke test xác nhận được luồng chính: ingest dữ liệu và gọi AI Engine thành công. Toàn bộ quá trình thực thi sẽ được xác minh thông qua bộ End-to-End (E2E) Test định kỳ.
...
Cổng destructive-change nghiêm ngặt hơn đối với các tài nguyên có lưu trạng thái (stateful). Các bucket S3, bảng DynamoDB, KMS key, các hàm Lambda, IAM role và bộ lưu trữ kiểm toán yêu cầu sự xác nhận của người xem xét khi có sự thay thế hoặc xóa xuất hiện trong plan. Các plan production phải bị hủy nếu chúng cố gắng terminate tài nguyên prod, xóa dữ liệu hoặc thay đổi IAM bên ngoài tập hợp module đã được phê duyệt.

### 3.3 AI contract compatibility

Trước khi cập nhật container image cho Lambda, pipeline sẽ khởi chạy một tập lệnh kiểm tra độ tương thích để thực thi **API contract phiên bản v1.3.0**:
1. Đối chiếu model version đăng ký từ AIOps với manifest image ECR mục tiêu.
2. Kiểm tra JSON schema của các API contract logic `/v1/detect` và `/v1/decide` của AI Engine (gọi trực tiếp qua Lambda), cụ thể xác thực:
   - Request payload bao gồm cờ boolean `telemetry_delay_event`, đích nhận `callback_url`, và trường `s3_bucket_uri` được kiểm tra khớp với biểu thức chính quy (regex) nghiêm ngặt `^s3://tf2-cdo[0-9]{2}-telemetry-[a-z0-9-]+/.*$`.
   - Response payload chứa phân loại `data_confidence` (LOW/MEDIUM/HIGH) và cấu trúc `rollback_payload.boto3_equivalent`.
...
- **AI Engine Lambda Workloads (Đồng bộ)**: Triển khai theo cơ chế **Lambda Weighted Aliases** bằng cách xuất bản các phiên bản Lambda và ghim mã băm ECR. Traffic được chuyển dịch dần dần: chạy thử nghiệm canary `10%` traffic trong 5 phút, và tự động chuyển sang `100%` nếu không phát sinh lỗi thực thi.
- **Alert Routing Lambda (SQS Trigger)**: Triển khai theo chiến lược **All-at-once** (triển khai toàn bộ một lần) cho việc đệm retry alert. SQS/DLQ chỉ được sử dụng cho hàng đợi retry của alert routing chứ không nằm trong luồng phát hiện. Độ an toàn được đảm bảo bằng cấu hình **Dead Letter Queue (DLQ)** cực kỳ nghiêm ngặt để hứng lỗi alert routing.
...
- **Kiểm soát đồng thời SQS (SQS Concurrency Controls)**: Được cấu hình trên mapping nguồn sự kiện (event source mapping) bằng các thiết lập số lượng thực thi đồng thời tối đa và giới hạn kích thước batch để căn chỉnh tiến trình xử lý thông báo và tránh cạn kiệt kết nối cơ sở dữ liệu.
- **Xử lý Timeout & Thử lại khi thực thi Lambda (Lambda Timeout & Retry)**: Hàm AI Engine Lambda được cấu hình với timeout an toàn và các quy tắc tự động thử lại. Đối với alert routing, SQS đóng vai trò là vùng đệm; nếu quá trình thực thi bị ngắt quãng, tin nhắn sẽ được đưa trở lại hàng đợi để thử lại tối đa trước khi đưa vào Dead Letter Queue (DLQ).
...
- **Rollback hạ tầng (Infrastructure Rollback)**: Rollback Terraform được xem xét qua plan thay vì chạy tự động. Các tài nguyên lưu trạng thái được bảo toàn, `prevent_destroy` vẫn được bật nếu được hỗ trợ và bất kỳ hoạt động rollback hạ tầng Lambda nào cũng phải tính đến cấu hình VPC và thiết lập SQS.
- **Kích hoạt Runbook (Runbook Trigger)**: Rollback được kích hoạt bởi smoke test thất bại, xác thực hợp đồng AI thất bại, tỷ lệ lỗi thực thi của Step Functions hoặc Lambda tăng cao, Lambda bị lỗi cold-start/timeout, hoặc dữ liệu dashboard cũ sau khi triển khai.
...
| **Staging** | Xác thực các container artifact của AIOps, gọi trực tiếp Lambda và chạy E2E pipeline của Step Functions. | `4444-5555-6666` | Đúng, từ `main` sau khi merge đã xem xét |
...
|---|---|---|
| **Log Aggregator** | CloudWatch Logs | Tập trung hóa nhật ký hoạt động từ application và Lambda container. |
| **Trace Analyzer** | AWS X-Ray | Tracing đường đi của request từ Step Functions, qua thực thi của AI Engine Lambda. |
| **Metrics Collector** | CloudWatch Metrics / X-Ray | Theo dõi thời gian thực thi Lambda, cold starts, số thực thi đồng thời, và tuổi hàng đợi SQS. |

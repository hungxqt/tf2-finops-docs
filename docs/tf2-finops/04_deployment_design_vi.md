...
Trong quá trình thiết lập cấu hình Terraform, module `s3-lakehouse` thực thi tiêu chuẩn `company-cdo-{account_id}-telemetry` cho việc tạo S3 bucket. Mọi định nghĩa tài nguyên bucket bắt buộc phải tuân theo quy tắc đặt tên này, sử dụng các biến cho ID tài khoản AWS `{account_id}` của thành viên nhằm ngăn chặn các không gian tên S3 không đạt chuẩn.
...
2. Chạy Lambda ingest với dữ liệu CUR/Cost Explorer.
...
7. Xác thực rằng CDO có thể chạy thành công một hoạt động rollback từ cache cục bộ trong khi AI Engine giả lập ở trạng thái offline, đảm bảo tính độc lập của rollback.
8. Xác thực ALB của AI Engine trả về phản hồi đồng bộ 200 OK trên '/health'.
```
...
Trước khi cập nhật container image cho Lambda, pipeline sẽ khởi chạy một tập lệnh kiểm tra độ tương thích để thực thi các API contract schema (ai-api-contract.md v1.4.0, telemetry-contract.md v3.2.0, deployment-contract.md v1.3.0):
...
| **Staging** | Xác thực các container artifact của AIOps, các API HTTPS ALB và chạy E2E pipeline của Step Functions. | `4444-5555-6666` | Đúng, từ `main` sau khi merge đã xem xét |
